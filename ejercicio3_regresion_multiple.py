...
def regresion_lineal_multiple(X_train, y_train, X_test):
    ...
    # Usamos lstsq para mayor estabilidad numérica
...
def graficar_real_vs_predicho(y_real, y_pred, ruta_salida="output/ej3_predicciones.png"):
    ...
    ax.scatter(y_real, y_pred, alpha=0.6, color="steelblue", s=20, label="Muestras")

    lim_min = min(y_real.min(), y_pred.min()) - 1
    lim_max = max(y_real.max(), y_pred.max()) + 1
    ax.plot([lim_min, lim_max], [lim_min, lim_max],
            "r--", linewidth=1.5, label="Predicción perfecta (y=x)")
    ax.set_xlim(lim_min, lim_max)
    ax.set_ylim(lim_min, lim_max)

    ax.set_xlabel("Valores Reales", fontsize=12)
    ax.set_ylabel("Valores Predichos", fontsize=12)
    ax.set_title("Real vs. Predicho — Regresión Lineal Múltiple (NumPy OLS)",
